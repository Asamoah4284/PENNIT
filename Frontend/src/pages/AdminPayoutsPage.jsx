...
export default function AdminPayoutsPage() {
  ...
  useEffect(() => {
    ...
    setLoading(true)
    setError(null)
    getAdminPayouts(user.id, { page, limit: PAGE_SIZE })
      .then((data) => {
  ...
  return (
    <div className="space-y-5 max-w-6xl">
      <div>
        <h1 className="text-[22px] font-bold text-[#111] tracking-tight">Payouts</h1>
        <p className="text-[13px] text-[#6B7280] mt-1">
          {loading ? '…' : `${total} payout${total !== 1 ? 's' : ''} total`}
        </p>
      </div>
      ...
      <div className="bg-white border border-[#E5E7EB] rounded-xl overflow-hidden overflow-x-auto">
        <table className="w-full text-left min-w-[560px]">
          <thead>
            <tr className="border-b border-[#F3F4F6] bg-[#FAFAFA]">
              {['Author', 'Month', 'Amount (GH₵)', 'Status', 'Paid at'].map((h) => (
            ...
            {loading ? (
              ...
              payouts.map((p) => (
                <tr key={p.id} className="hover:bg-[#FAFAFA]">
                  <td className="px-5 py-3.5 text-[13px] font-medium text-[#111]">{p.authorPenName || '—'}</td>
                  <td className="px-5 py-3.5 text-[13px] text-[#6B7280]">{p.month || '—'}</td>
        ...
        {!loading && payouts.length === 0 && (
          <div className="py-16 text-center text-[13px] text-[#9CA3AF]">No payouts yet.</div>
        )}
      </div>
